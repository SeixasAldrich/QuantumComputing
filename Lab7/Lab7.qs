...
namespace Lab7 {
    ...
    operation Exercise4_GroverIteration (
        ...
        ApplyToEach(X, register);
        Controlled Z(register[1..(length - 1)], register[0]);
        ApplyToEach(X, register);
        ApplyToEach(H, register);
    ...
    operation Exercise5_GroverSearch (
        ...
        for i in 0 .. iterations - 1 {
            ApplyToEach(H, register);
            ApplyToEach(X, register);
            Controlled Z(register[1..(numberOfQubits - 1)], register[0]);
            ApplyToEach(X, register);
            ApplyToEach(H, register);
        }
        ...
        mutable output = [false,size=numberOfQubits];
