...
namespace Lab9 {
    ...
    operation Exercise1_ModExp (
        ...
        // for i in 0 .. len - 1 {
        //     Microsoft.Quantum.Math.ExpModI(a, , b);
        //     set output[i] = Microsoft.Quantum.Arithmetic.MultiplyByModularInteger(a, b, input[i]);
        // }
    ...
    operation Exercise2_FindApproxPeriod (
        ...
        let little_n = numberToFactor ^ 2;

        use register_I = Qubit[little_n];
        use register_O = Qubit[little_n];

        ApplyToEach(H, register_I);

        Exercise1_ModExp(guess, numberToFactor, register_O, register_O);
    ...
    function Exercise3_FindPeriodCandidate (
        ...
        mutable curr_num = numerator;
        mutable curr_den = denominator;
        mutable i = 0;
        mutable quotient = 0;
        mutable remainder = 0;

        while curr_den >= denominatorThreshold {
            set quotient = curr_num / curr_den;
            set remainder = curr_num % curr_den;
            set curr_num = curr_den;
            set curr_den = remainder;
            set i = i + 1;
        }
    ...
    function Exercise5_FindFactor (
        ...
        let check = ((guess ^ (period / 2)) + 1) % numberToFactor;
        ...
        let factor1 = Microsoft.Quantum.Math.GreatestCommonDivisorI(((guess ^ (period / 2)) + 1), numberToFactor);
        let factor2 = Microsoft.Quantum.Math.GreatestCommonDivisorI(((guess ^ (period / 2)) - 1), numberToFactor);
