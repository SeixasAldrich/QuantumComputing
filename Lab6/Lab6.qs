...
namespace Lab6 {
    ...
    operation Exercise1 (
        ...
        use qubitIn = Qubit[length];
        use qubitOut = Qubit[length];
        mutable output = [true,size=length];
        mutable count = 0;
        for bit in input {
            if(bit == true) {
                X(qubitIn[count]);
        ...
        set count = 0;
        op(qubitIn, qubitOut);
        for qubit in qubitOut {
            set output w/= count <- M(qubit) == One ? true | false;
            set count = count + 1;
        }
    ...
    operation Exercise2 (
        ...
        use qubitIn = Qubit[inputSize];
        use qubitOut = Qubit[inputSize];
        mutable output = [true, size=inputSize];
        ApplyToEach(H, qubitIn);
        op(qubitIn, qubitOut);
        ApplyToEach(H, qubitIn);
        mutable count = 0; 
        for qubit in qubitIn {
            set output w/= count <- M(qubit) == One ? true | false;
            set count = count + 1;
        }
