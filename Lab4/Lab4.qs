...
namespace Lab4 {
    ...
    operation Exercise2 (pairA : Qubit, pairB : Qubit) : Bool[] {
        CNOT(pairA, pairB);
        H(pairA);
        mutable bits = [false, false];
        let resultAsInt1 = M(pairA) == One ? true | false;
        let resultAsInt2 = M(pairB) == One ? true | false;
        set bits w/= 0 <- resultAsInt1;
        set bits w/= 1 <- resultAsInt2;
        return bits;
    }
    ...
    operation Exercise4 (
        ...
        mutable secretVal = false;
        mutable canUse = false;

        if ((aPublic == bPublic) and (aPublic == true)){
            H(qubit);
            set canUse = true;
            set secretVal = M(qubit) == One ? true | false;
        } elif ((aPublic == bPublic) and (aPublic == false)) {
            set canUse = true;
            set secretVal = M(qubit) == One ? true | false;
        }

        return (secretVal, canUse);
    }
}
